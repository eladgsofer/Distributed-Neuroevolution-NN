...
bringGeneToLife(FileName, Genotype)->
	...
	% Initialize entities
	triggerAndInitNN_Component(CerebralUnits,IdsNPIds),
	triggerAndInitCortex(Cx,IdsNPIds), 
...
triggerAndInitNN_Component([R|Records],IdsNPIds) when is_record(R,sensor) ->
	SId = R#sensor.id,
	...
	triggerAndInitNN_Component(Records,IdsNPIds);

triggerAndInitNN_Component([R|Records],IdsNPIds) when is_record(R,actuator) ->
	AId = R#actuator.id,
...
triggerAndInitNN_Component([R|Records],IdsNPIds) when is_record(R,neuron) ->
	NId = R#neuron.id,
...
triggerAndInitNN_Component([],_IdsNPIds)-> ok.
