...
%The function create_NeuroLayers/3 prepares the initial step before starting the recursive create_NeuroLayers/7 function which will create all the Neuron records.
% We first generate the place holder Input Ids “plus”(Input_IdPs), which are tuples composed of Ids and the vector lengths of the incoming signals associated with them.
% The proper input_idps will have a weight list in the tuple instead of the vector length. Because we are only building NNs each with only a single Sensor and Actuator,
% the IdP to the first layer is composed of the single Sensor Id with the vector length of its sensory signal, likewise in the case of the Actuator.
% We then generate unique ids for the neurons in the first layer, and drop into the recursive create_NeuroLayers/7 function.
