...
loop(ExoSelf_PId) ->
	receive
		{ExoSelf_PId,{Id,SPIds,APIds,NPIds},TotSteps} ->
			put(start_time,now()),
			Init_loc=[?HUNTER_INIT_LOC, ?HUNTER_INIT_LOC],
...
loop(Id,ExoSelf_PId,SPIds,{_APIds,MAPIds},NPIds,0, {HunterLoc, DistanceAcc}) ->
	...
	ExoSelf_PId ! {self(),score_and_backup,{Neuron_IdsNWeights, Fitness_Score}},
	% Terminating all the network
	[PId ! {self(),terminate} || PId <- SPIds],
	[PId ! {self(),terminate} || PId <- MAPIds],
	[PId ! {self(),termiante} || PId <- NPIds];

loop(Id,ExoSelf_PId,SPIds,{[APId|APIds],MAPIds},NPIds,Step, {_, DistanceAcc}) ->
	receive
		{APId,sync,HunterLoc} ->
			CurrDist = calcDistance(Step, HunterLoc),
			loop(Id,ExoSelf_PId,SPIds,{APIds,MAPIds},NPIds,Step,{HunterLoc, CurrDist+DistanceAcc});
		terminate ->
...
loop(Id,ExoSelf_PId,SPIds,{[],MAPIds},NPIds,Step, {Hunter_loc, DistanceAcc})->

	[PId ! {self(),sync,Hunter_loc} || PId <- SPIds],
	loop(Id,ExoSelf_PId,SPIds,{MAPIds,MAPIds},NPIds,Step-1,{Hunter_loc, DistanceAcc}).
...
calcDistance(Step, HunterLoc)->
	...
	io:format("STEP:~p DISTANCE:~p~n", [Step, Distance]), Distance.
